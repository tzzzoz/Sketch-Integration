...
- (id)initWithImage:(UIImage *)i{
    ...
    if (self) {
        ...
        imageData=[ImageHelper convertUIImageToBitmapRGBA8:image];
    }
    return self;
...
-(BOOL)isPixelValid:(int)x andY:(int)y{
    if([self isIndexValid:x andY:y]){
        ...
        /*if((targetColor.red==red&&targetColor.green==green&&targetColor.blue==blue
           &&targetColor.alpha==alpha)||(boundaryColor.red==red&&boundaryColor.green==green&&boundaryColor.blue==blue&&boundaryColor.alpha==alpha))
            return false;
        else
            return true;*/
...
-(void) ScanLineSeedFill:(int)x andY:(int)y withTC:(struct ColorRGBAStruct)tc andBC:(struct ColorRGBAStruct)bc
{
    ...
    while(![myStack isEmpty])
    {
        ...
        //第3步，向左右填充
        int count = [self FillLineRight:seedX andY:seedY];//向'cf?右'd3?填'cc?充'b3?
        int xRight = seedX + count - 1;
        count = [self FillLineLeft:seedX-1 andY:seedY];//向'cf?左'd7?填'cc?充'b3?
